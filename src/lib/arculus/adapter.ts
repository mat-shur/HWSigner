...
import { ArculusWalletConnectClient } from '@/lib/arculus/client';

export function createArculusAdapter(runtime: HWSignerRuntime, onEvent?: HWSignerEventListener): WalletAdapter {
  if (runtime.kind !== 'arculus-walletconnect') {
    throw new UnsupportedOperationError('Arculus only supports the WalletConnect runtime in this project.');
  }

  return new ArculusAdapter(onEvent);
}

class ArculusAdapter implements WalletAdapter {
  private readonly client: ArculusWalletConnectClient;

  constructor(onEvent?: HWSignerEventListener) {
    this.client = new ArculusWalletConnectClient(onEvent);
  }
