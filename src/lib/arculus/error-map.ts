...
export function mapArculusError(error: unknown): HWSignerError {
  ...
  const message = getArculusErrorMessage(error);
  const normalized = message.toLowerCase();
  const name = getArculusErrorName(error);
...
export function getArculusErrorMessage(error: unknown): string {
  ...
  return 'Arculus request failed.';
}

function getArculusErrorName(error: unknown): string {
