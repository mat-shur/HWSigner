...
export class EllipalWalletConnectClient {
  ...
  async connect(): Promise<HWSignerConnection> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Opening ELLIPAL WalletConnect QR flow.',
    });

    try {
      await adapter.connect();
    } catch (error) {
      throw mapEllipalError(error);
    }

    const publicKey = adapter.publicKey?.toBase58();
    if (!publicKey) {
      throw new DeviceConnectionError('ELLIPAL did not return a public key.');
    }
  ...
  async signMessage(input: SignMessageInput): Promise<SignedMessageResult> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting ELLIPAL message signature over WalletConnect.',
    });
    ...
    return {
      address,
      derivationPath: ELLIPAL_ACCOUNT_PATH,
  ...
  async signTransaction(input: SignTransactionInput): Promise<SignedTransactionResult> {
    if (input.signingPayloadMode && input.signingPayloadMode !== 'serialized-transaction') {
      throw new UnsupportedOperationError('ELLIPAL only supports "serialized-transaction" signing in this runtime.');
    }
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting ELLIPAL signature for a legacy transaction.',
    });
    ...
    if (!signature) {
      throw new DeviceConnectionError('ELLIPAL did not return a transaction signature.');
    }
    ...
    return buildSignedLegacyResult({
      ...
      address,
      derivationPath: input.derivationPath ?? ELLIPAL_ACCOUNT_PATH,
  ...
  async signVersionedTransaction(input: SignVersionedTransactionInput): Promise<SignedTransactionResult> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting ELLIPAL signature for a versioned transaction.',
    });
    ...
    if (!signature || signature.length === 0) {
      throw new DeviceConnectionError('ELLIPAL did not return a versioned transaction signature.');
    }
  ...
  private async requireConnection(): Promise<{ adapter: WalletConnectAdapter; address: string }> {
    ...
    if (!address) {
      throw new DeviceConnectionError('ELLIPAL is not connected.');
    }
  ...
  private ensureBrowser() {
    if (typeof window === 'undefined') {
      throw new UnsupportedOperationError('ELLIPAL WalletConnect can only run in a browser.');
    }
  }
}
...
function getWalletConnectProjectId(): string {
  ...
  if (!projectId) {
    throw new AdapterInitializationError('NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID is required for the ELLIPAL WalletConnect runtime.');
  }
...
export function normalizeEllipalErrorMessage(error: unknown): string {
  return getEllipalErrorMessage(error);
}