...
import { getTangemErrorMessage, mapTangemError } from '@/lib/tangem/error-map';

const TANGEM_WALLETCONNECT_ACCOUNT_PATH = 'tangem://walletconnect-selected-account';
...
export class TangemWalletConnectClient {
  ...
  async connect(): Promise<HWSignerConnection> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Opening Tangem WalletConnect flow.',
    });
    ...
    if (!publicKey) {
      throw new DeviceConnectionError('Tangem did not return a public key.');
    }
  ...
  async getAccounts(input: GetAccountsInput) {
    ...
    return [{
      index: 0,
      path: TANGEM_WALLETCONNECT_ACCOUNT_PATH,
      address,
    }];
  }
  ...
  async signMessage(input: SignMessageInput): Promise<SignedMessageResult> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting Tangem message signature over WalletConnect.',
    });
    ...
    return {
      address,
      derivationPath: TANGEM_WALLETCONNECT_ACCOUNT_PATH,
  ...
  async signTransaction(input: SignTransactionInput): Promise<SignedTransactionResult> {
    if (input.signingPayloadMode && input.signingPayloadMode !== 'serialized-transaction') {
      throw new UnsupportedOperationError('Tangem WalletConnect only supports "serialized-transaction" signing in this runtime.');
    }
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting Tangem signature for a legacy transaction over WalletConnect.',
    });
    ...
    if (!signature) {
      throw new DeviceConnectionError('Tangem did not return a transaction signature.');
    }
    ...
    return buildSignedLegacyResult({
      ...
      address,
      derivationPath: input.derivationPath ?? TANGEM_WALLETCONNECT_ACCOUNT_PATH,
  ...
  async signVersionedTransaction(input: SignVersionedTransactionInput): Promise<SignedTransactionResult> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting Tangem signature for a versioned transaction over WalletConnect.',
    });
    ...
    if (!signature || signature.length === 0) {
      throw new DeviceConnectionError('Tangem did not return a versioned transaction signature.');
    }
  ...
  private async requireConnection(): Promise<{ adapter: WalletConnectAdapter; address: string }> {
    ...
    if (!address) {
      throw new DeviceConnectionError('Tangem WalletConnect is not connected.');
    }
  ...
  private ensureBrowser() {
    if (typeof window === 'undefined') {
      throw new UnsupportedOperationError('Tangem WalletConnect can only run in a browser.');
    }
  }
}
...
function getWalletConnectProjectId(): string {
  ...
  if (!projectId) {
    throw new AdapterInitializationError('NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID is required for the Tangem WalletConnect runtime.');
  }
...
export function normalizeTangemWalletConnectErrorMessage(error: unknown): string {
  return getTangemErrorMessage(error);
}
