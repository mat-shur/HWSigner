...
export function mapBcVaultError(error: unknown): HWSignerError {
  if (error instanceof HWSignerError) {
    return error;
  }

  const message = getBcVaultErrorMessage(error);
  const normalized = message.toLowerCase();
  const name = getBcVaultErrorName(error);
...
export function getBcVaultErrorMessage(error: unknown): string {
  ...
  return 'BC Vault request failed.';
}

function getBcVaultErrorName(error: unknown): string {
