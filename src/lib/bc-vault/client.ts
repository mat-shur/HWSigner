...
const BC_VAULT_ACCOUNT_PATH = 'bc-vault://selected-account';
...
export class BcVaultWalletConnectClient {
  ...
  async connect(): Promise<HWSignerConnection> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Opening BC Vault WalletConnect flow.',
    });

    try {
      await adapter.connect();
    } catch (error) {
      throw mapBcVaultError(error);
    }

    const publicKey = adapter.publicKey?.toBase58();
    if (!publicKey) {
      throw new DeviceConnectionError('BC Vault did not return a public key.');
    }
  ...
  async signMessage(input: SignMessageInput): Promise<SignedMessageResult> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting BC Vault message signature over WalletConnect.',
    });
    ...
    return {
      address,
      derivationPath: BC_VAULT_ACCOUNT_PATH,
  ...
  async signTransaction(input: SignTransactionInput): Promise<SignedTransactionResult> {
    if (input.signingPayloadMode && input.signingPayloadMode !== 'serialized-transaction') {
      throw new UnsupportedOperationError('BC Vault only supports "serialized-transaction" signing in this WalletConnect runtime.');
    }
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting BC Vault signature for a legacy transaction.',
    });
    ...
    if (!signature) {
      throw new DeviceConnectionError('BC Vault did not return a transaction signature.');
    }
    ...
    return buildSignedLegacyResult({
      ...
      address,
      derivationPath: input.derivationPath ?? BC_VAULT_ACCOUNT_PATH,
  ...
  async signVersionedTransaction(input: SignVersionedTransactionInput): Promise<SignedTransactionResult> {
    ...
    this.onEvent?.({
      type: 'action',
      message: 'Requesting BC Vault signature for a versioned transaction.',
    });
    ...
    if (!signature || signature.length === 0) {
      throw new DeviceConnectionError('BC Vault did not return a versioned transaction signature.');
    }
  ...
  private async requireConnection(): Promise<{ adapter: WalletConnectAdapter; address: string }> {
    ...
    if (!address) {
      throw new DeviceConnectionError('BC Vault is not connected.');
    }
  ...
  private ensureBrowser() {
    if (typeof window === 'undefined') {
      throw new UnsupportedOperationError('BC Vault WalletConnect can only run in a browser.');
    }
  }
}
...
function getWalletConnectProjectId(): string {
  ...
  if (!projectId) {
    throw new AdapterInitializationError('NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID is required for the BC Vault WalletConnect runtime.');
  }
...
export function normalizeBcVaultErrorMessage(error: unknown): string {
  return getBcVaultErrorMessage(error);
}
