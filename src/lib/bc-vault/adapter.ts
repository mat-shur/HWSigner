...
import { BcVaultWalletConnectClient } from '@/lib/bc-vault/client';

export function createBcVaultAdapter(runtime: HWSignerRuntime, onEvent?: HWSignerEventListener): WalletAdapter {
  if (runtime.kind !== 'bc-vault-walletconnect') {
    throw new UnsupportedOperationError('BC Vault only supports the WalletConnect runtime in this project.');
  }

  return new BcVaultAdapter(onEvent);
}

class BcVaultAdapter implements WalletAdapter {
  private readonly client: BcVaultWalletConnectClient;

  constructor(onEvent?: HWSignerEventListener) {
    this.client = new BcVaultWalletConnectClient(onEvent);
  }
