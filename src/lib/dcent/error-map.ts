...
export function mapDcentError(error: unknown): HWSignerError {
  if (error instanceof HWSignerError) {
    return error;
  }

  const message = getDcentErrorMessage(error);
  const normalized = message.toLowerCase();
  const name = getDcentErrorName(error);
...
export function getDcentErrorMessage(error: unknown): string {
  ...
  return "D'CENT request failed.";
}

function getDcentErrorName(error: unknown): string {
