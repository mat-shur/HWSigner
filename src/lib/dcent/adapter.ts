...
export function createDcentAdapter(runtime: HWSignerRuntime, onEvent?: HWSignerEventListener): WalletAdapter {
  if (runtime.kind !== 'dcent-walletconnect') {
    throw new UnsupportedOperationError("D'CENT only supports the WalletConnect QR runtime in this project.");
  }

  return new DcentAdapter(onEvent);
}

class DcentAdapter implements WalletAdapter {
  private readonly client: DcentWalletConnectClient;

  constructor(onEvent?: HWSignerEventListener) {
    this.client = new DcentWalletConnectClient(onEvent);
  }
